...
INSERT INTO archive VALUES ('843ded531810400abdfd04296d387c3c', 'ARTS19-20181111', 'Jason Yang', '# ARTS19-20181111
...
', '<h1 id="arts19-20181111">ARTS19-20181111</h1>
<blockquote>
<p>这期各种日志相关。。。</p>
</blockquote>
<h2 id="algorithm">Algorithm</h2>
<h3 id="-937-reorder-log-files-https-leetcode-com-problems-reorder-log-files-"><a href="https://leetcode.com/problems/reorder-log-files/">937. Reorder Log Files</a></h3>
...
<h3 id="-30-best-practices-for-logging-at-scale-https-www-loggly-com-blog-30-best-practices-logging-scale-"><a href="https://www.loggly.com/blog/30-best-practices-logging-scale/">30 best practices for logging at scale</a></h3>
<p>一篇来自 Jason Skowronski 的文章，大规模记录日志的一些最佳实践，包括日志的创建，传输，以及日志的管理，我主要理解了一下创建日志的最佳做法。</p>
<ul>
<li>创建日志的最佳做法<ul>
<li>使用标准且易于配置的日志框架。<ul>
<li>log4j，log4net等允许比硬编码或专有框架更快的配置更改。</li>
</ul>
</li>
<li>使用具有灵活输出选项的日志框架。<ul>
<li>查看控制台登录开发并集中prod日志，无需额外的插件或代理。</li>
</ul>
</li>
<li>使用像JSON这样的标准结构化格式。<ul>
<li>自定义格式和原始文本日志需要自定义解析规则来提取数据以进行分析。</li>
</ul>
</li>
<li>Create a standard schema for your fields.（没理解）<ul>
<li>Adding fields ad hoc can create a rat’s nest. A standard lets everyone know where to look.</li>
</ul>
</li>
<li>不要让日志记录阻止您的应用程序。<ul>
<li>使用缓冲区或队列异步写入日志，以便应用程序可以继续运行。</li>
</ul>
</li>
<li>注意平台即服务（PaaS）或基于容器的环境中的限制。<ul>
...
<h2 id="tip">Tip</h2>
<h3 id="ubuntu-">Ubuntu 启动界面修改</h3>
<pre><code class="lang-shell">sudo update-alternatives --config default.plymouth
sudo update-initramfs -u
</code></pre>
<h3 id="-canal-message-">线程切换导致 Canal Message 乱序消费问题</h3>
...
<p><a href="https://github.com/yangjinlong86/arts/blob/master/2018/about-logfile.md">关于日志的那些事儿</a><br>整理了关于日志的一些内容，耗叔要求的【目标是一篇“引经据典“的，“全面系统“的对日志的介绍】。结合一些参考链接和以往的经验，主要对软件开发过程中日志相关的知识进行梳理和学习。</p>
', '这期各种日志相关。。。', 'arts', NULL, NULL, 2, '2018-11-11 16:30:33.946+00', NULL);
...
INSERT INTO archive VALUES ('6960276b5fab468f93d49f557245e76d', 'Apache Kafka Quickstart', 'apache', '> This tutorial assumes you are starting fresh and have no existing Kafka or ZooKeeper data. Since Kafka console scripts are different for Unix-based and Windows platforms, on Windows platforms use `bin\windows\` instead of `bin/`, and change the script extension to `.bat`.
...
Kafka Streams is a client library for building mission-critical real-time applications and microservices, where the input and/or output data is stored in Kafka clusters. Kafka Streams combines the simplicity of writing and deploying standard Java and Scala applications on the client side with the benefits of Kafka''s server-side cluster technology to make these applications highly scalable, elastic, fault-tolerant, distributed, and much more. This [quickstart example](https://kafka.apache.org/20/documentation/streams/quickstart) will demonstrate how to run a streaming application coded in this library.', '<blockquote>
<p>This tutorial assumes you are starting fresh and have no existing Kafka or ZooKeeper data. Since Kafka console scripts are different for Unix-based and Windows platforms, on Windows platforms use <code>bin\windows\</code> instead of <code>bin/</code>, and change the script extension to <code>.bat</code>.</p>
</blockquote>
<h4 id="-step-1-download-the-code-https-kafka-apache-org-quickstart-quickstart_download-"><a href="https://kafka.apache.org/quickstart#quickstart_download">Step 1: Download the code</a></h4>
<p>Download the 2.0.0 release and un-tar it.</p>
<pre><code>&gt; ``tar` `-xzf kafka_2.11-2.0.0.tgz``&gt; ``cd` `kafka_2.11-2.0.0
</code></pre><h4 id="-step-2-start-the-server-https-kafka-apache-org-quickstart-quickstart_startserver-"><a href="https://kafka.apache.org/quickstart#quickstart_startserver">Step 2: Start the server</a></h4>
...
<pre><code>&gt; bin``/zookeeper-server-start``.sh config``/zookeeper``.properties``[2013-04-22 15:01:37,495] INFO Reading configuration from: config``/zookeeper``.properties (org.apache.zookeeper.server.quorum.QuorumPeerConfig)``...
</code></pre><p>Now start the Kafka server:</p>
<pre><code>&gt; bin``/kafka-server-start``.sh config``/server``.properties``[2013-04-22 15:01:47,028] INFO Verifying properties (kafka.utils.VerifiableProperties)``[2013-04-22 15:01:47,051] INFO Property socket.send.buffer.bytes is overridden to 1048576 (kafka.utils.VerifiableProperties)``...
</code></pre><h4 id="-step-3-create-a-topic-https-kafka-apache-org-quickstart-quickstart_createtopic-"><a href="https://kafka.apache.org/quickstart#quickstart_createtopic">Step 3: Create a topic</a></h4>
...
</code></pre><p>We can now see that topic if we run the list topic command:</p>
<pre><code>&gt; bin``/kafka-topics``.sh --list --zookeeper localhost:2181``test
</code></pre><p>Alternatively, instead of manually creating topics you can also configure your brokers to auto-create topics when a non-existent topic is published to.</p>
<h4 id="-step-4-send-some-messages-https-kafka-apache-org-quickstart-quickstart_send-"><a href="https://kafka.apache.org/quickstart#quickstart_send">Step 4: Send some messages</a></h4>
...
<pre><code>&gt; bin``/kafka-console-producer``.sh --broker-list localhost:9092 --topic ``test``This is a message``This is another message
</code></pre><h4 id="-step-5-start-a-consumer-https-kafka-apache-org-quickstart-quickstart_consume-"><a href="https://kafka.apache.org/quickstart#quickstart_consume">Step 5: Start a consumer</a></h4>
<p>Kafka also has a command line consumer that will dump out messages to standard output.</p>
<pre><code>&gt; bin``/kafka-console-consumer``.sh --bootstrap-server localhost:9092 --topic ``test` `--from-beginning``This is a message``This is another message
</code></pre><p>If you have each of the above commands running in a different terminal then you should now be able to type messages into the producer terminal and see them appear in the consumer terminal.</p>
...
<h4 id="-step-6-setting-up-a-multi-broker-cluster-https-kafka-apache-org-quickstart-quickstart_multibroker-"><a href="https://kafka.apache.org/quickstart#quickstart_multibroker">Step 6: Setting up a multi-broker cluster</a></h4>
...
<p>First we make a config file for each of the brokers (on Windows use the <code>copy</code> command instead):</p>
<pre><code>&gt; ``cp` `config``/server``.properties config``/server-1``.properties``&gt; ``cp` `config``/server``.properties config``/server-2``.properties
</code></pre><p>Now edit these new files and set the following properties:</p>
<pre><code>config/server-1.properties:``    ``broker.id=1``    ``listeners=PLAINTEXT://:9093``    ``log.dirs=/tmp/kafka-logs-1` `config/server-2.properties:``    ``broker.id=2``    ``listeners=PLAINTEXT://:9094``    ``log.dirs=/tmp/kafka-logs-2
...
<pre><code>&gt; bin``/kafka-server-start``.sh config``/server-1``.properties &amp;``...``&gt; bin``/kafka-server-start``.sh config``/server-2``.properties &amp;``...
</code></pre><p>Now create a new topic with a replication factor of three:</p>
<pre><code>&gt; bin``/kafka-topics``.sh --create --zookeeper localhost:2181 --replication-factor 3 --partitions 1 --topic my-replicated-topic
</code></pre><p>Okay but now that we have a cluster how can we know which broker is doing what? To see that run the &quot;describe topics&quot; command:</p>
<pre><code>&gt; bin``/kafka-topics``.sh --describe --zookeeper localhost:2181 --topic my-replicated-topic``Topic:my-replicated-topic   PartitionCount:1    ReplicationFactor:3 Configs:``    ``Topic: my-replicated-topic  Partition: 0    Leader: 1   Replicas: 1,2,0 Isr: 1,2,0
...
<pre><code>&gt; bin``/kafka-topics``.sh --describe --zookeeper localhost:2181 --topic ``test``Topic:``test`  `PartitionCount:1    ReplicationFactor:1 Configs:``    ``Topic: ``test` `Partition: 0    Leader: 0   Replicas: 0 Isr: 0
</code></pre><p>So there is no surprise there—the original topic has no replicas and is on server 0, the only server in our cluster when we created it.</p>
<p>Let&#39;s publish a few messages to our new topic:</p>
<pre><code>&gt; bin``/kafka-console-producer``.sh --broker-list localhost:9092 --topic my-replicated-topic``...``my ``test` `message 1``my ``test` `message 2``^C
</code></pre><p>Now let&#39;s consume these messages:</p>
<pre><code>&gt; bin``/kafka-console-consumer``.sh --bootstrap-server localhost:9092 --from-beginning --topic my-replicated-topic``...``my ``test` `message 1``my ``test` `message 2``^C
</code></pre><p>Now let&#39;s test out fault-tolerance. Broker 1 was acting as the leader so let&#39;s kill it:</p>
<pre><code>&gt; ``ps` `aux | ``grep` `server-1.properties``7564 ttys002    0:15.91 ``/System/Library/Frameworks/JavaVM``.framework``/Versions/1``.8``/Home/bin/java``...``&gt; ``kill` `-9 7564
</code></pre><p>On Windows use:</p>
<pre><code>&gt; wmic process where ``&quot;caption = &#39;java.exe&#39; and commandline like &#39;%server-1.properties%&#39;&quot;` `get processid``ProcessId``6016``&gt; taskkill ``/pid` `6016 ``/f
</code></pre><p>Leadership has switched to one of the slaves and node 1 is no longer in the in-sync replica set:</p>
<pre><code>&gt; bin``/kafka-topics``.sh --describe --zookeeper localhost:2181 --topic my-replicated-topic``Topic:my-replicated-topic   PartitionCount:1    ReplicationFactor:3 Configs:``    ``Topic: my-replicated-topic  Partition: 0    Leader: 2   Replicas: 1,2,0 Isr: 2,0
</code></pre><p>But the messages are still available for consumption even though the leader that took the writes originally is down:</p>
<pre><code>&gt; bin``/kafka-console-consumer``.sh --bootstrap-server localhost:9092 --from-beginning --topic my-replicated-topic``...``my ``test` `message 1``my ``test` `message 2``^C
</code></pre><h4 id="-step-7-use-kafka-connect-to-import-export-data-https-kafka-apache-org-quickstart-quickstart_kafkaconnect-"><a href="https://kafka.apache.org/quickstart#quickstart_kafkaconnect">Step 7: Use Kafka Connect to import/export data</a></h4>
...
<pre><code>&gt; ``echo` `-e ``&quot;foo\nbar&quot;` `&gt; ``test``.txt
</code></pre><p>Or on Windows:</p>
<pre><code>&gt; ``echo` `foo&gt; ``test``.txt``&gt; ``echo` `bar&gt;&gt; ``test``.txt
</code></pre><p>Next, we&#39;ll start two connectors running in <em>standalone</em> mode, which means they run in a single, local, dedicated process. We provide three configuration files as parameters. The first is always the configuration for the Kafka Connect process, containing common configuration such as the Kafka brokers to connect to and the serialization format for data. The remaining configuration files each specify a connector to create. These files include a unique connector name, the connector class to instantiate, and any other configuration required by the connector.</p>
...
<pre><code>&gt; ``more` `test``.sink.txt``foo``bar
</code></pre><p>Note that the data is being stored in the Kafka topic <code>connect-test</code>, so we can also run a console consumer to see the data in the topic (or use custom consumer code to process it):</p>
<pre><code>&gt; bin``/kafka-console-consumer``.sh --bootstrap-server localhost:9092 --topic connect-``test` `--from-beginning``{``&quot;schema&quot;``:{``&quot;type&quot;``:``&quot;string&quot;``,``&quot;optional&quot;``:``false``},``&quot;payload&quot;``:``&quot;foo&quot;``}``{``&quot;schema&quot;``:{``&quot;type&quot;``:``&quot;string&quot;``,``&quot;optional&quot;``:``false``},``&quot;payload&quot;``:``&quot;bar&quot;``}``...
</code></pre><p>The connectors continue to process data, so we can add data to the file and see it move through the pipeline:</p>
<pre><code>&gt; ``echo` `Another line&gt;&gt; ``test``.txt
</code></pre><p>You should see the line appear in the console consumer output and in the sink file.</p>
<h4 id="-step-8-use-kafka-streams-to-process-data-https-kafka-apache-org-quickstart-quickstart_kafkastreams-"><a href="https://kafka.apache.org/quickstart#quickstart_kafkastreams">Step 8: Use Kafka Streams to process data</a></h4>
...
', 'This tutorial assumes you are starting fresh and have no existing Kafka or ZooKeeper data. ', 'kafka', NULL, NULL, 2, '2018-10-10 07:36:54.363+01', NULL);
INSERT INTO archive VALUES ('65e15754165446728067348c2133f60c', 'ARTS16-20181021', 'Jason Yang', '# ARTS16-20181021
...
<h2 id="review">Review</h2>
<p><a href="http://cloudurable.com/blog/kafka-tutorial-kafka-producer/index.html">http://cloudurable.com/blog/kafka-tutorial-kafka-producer/index.html</a><br><a href="https://dzone.com/articles/writing-a-kafka-consumer-in-java">https://dzone.com/articles/writing-a-kafka-consumer-in-java</a></p>
...
<h2 id="tip">Tip</h2>
<h3 id="-jenkins-pipeline-python-">使用 jenkins pipeline 定时运行python脚本</h3>
<blockquote>
...
<li>注意权限，jenkins项目 是用 <code>jenkins</code> 用户来执行命令的</li>
...
', '第十六周ARTS。这个周末也不消停，一家人都得了感冒，前两天吃了药就会很困，实在坚持不住。乘着这会儿孩子睡着了，赶紧把ARTS写了，这一周已经积累了arts的素材，应该不会耗时太久。', 'arts', NULL, NULL, 2, '2018-10-28 15:02:27.497+00', NULL);
...
INSERT INTO archive VALUES ('6b41915212e1418495f35fd72cd2cb98', 'ARTS14-20181006', 'Jason Yang', '
...
<h2 id="review">Review</h2>
<h3 id="-cap-theorem-https-en-wikipedia-org-wiki-cap_theorem-"><a href="https://en.wikipedia.org/wiki/CAP_theorem">CAP theorem</a></h3>
<p><a href="https://time.geekbang.org/column/article/2080">左耳听风-分布式系统架构经典资料</a></p>
<ul>
<li><em>Consistency</em>: Every read receives the most recent write or an error<ul>
<li>一致性：每次读取要么获得最近写入的数据，要么获得一个错误</li>
</ul>
</li>
<li><em>Availability</em>: Every request receives a response that is not an error<ul>
<li>可用性：每次请求都能获得一个响应，并非一个错误</li>
</ul>
</li>
<li><em>Partition tolerance</em>: The system continues to operate despite an arbitrary number of messages being dropped (or delayed) by the network between nodes<ul>
...
<h2 id="share">Share</h2>
<h3 id="-http-nocoder-org-archive-27-"><a href="http://nocoder.org/archive/27">不能改变，那就接受</a></h3>
<blockquote>
<p>分享一篇去年写的关于抱怨的短文</p>
</blockquote>
...
', '第十四周ARTS。国庆节接近尾声了，这个十一过得有点痛苦，给孩子断奶、孩子发烧，折腾了5天，心疼宝宝啊，感慨下为人父母的不容易', 'arts', NULL, NULL, 2, '2018-10-13 18:02:28.889+01', NULL);
INSERT INTO archive VALUES ('d3df61cd6db841d0a4e94c809bbff499', 'ARTS20-20181118', 'Jason Yang', '# ARTS20-20181118
...
', '<h1 id="arts20-20181118">ARTS20-20181118</h1>
<h2 id="algorithm">Algorithm</h2>
<pre><code class="lang-java">package org.nocoder.leetcode.solution;
...
<pre><code class="lang-java">ThreadPoolExecutor executor = new ThreadPoolExecutor(
    N_THREADS, N_THREADS, 0L, TimeUnit.MILLISECONDS,
    new LinkedBlockingQueue&lt;Runnable&gt;(CAPACITY));
...
<li>对于包含IO操作或者其他阻塞操作的任务，由于线程并不会一直执行，因此线程池的规模应该更大<ul>
<li>估算出任务的等待时间于计算时间的比值    <ul>
<li>在某个基准负载下，分别设置不同大小的线程池来运行程序，观察CPU利用率</li>
<li>N = CPU的数量(可以通过<code>Runtime.getRuntime().availableProcessors()</code>获取CPU数量)</li>
<li>U = CPU的使用率(0 &lt;= U &lt;= 1）</li>
<li>W/C = 计算等待时间 / 计算时间</li>
<li>线程池的最优大小 = N <em> U </em> (1 + W/C)</li>
</ul>
</li>
</ul>
</li>
</ul>
', '本期各种线程相关。。。', 'arts', NULL, NULL, 2, '2018-11-21 07:16:55.634+00', NULL);
...
INSERT INTO archive VALUES ('feb65a89c65a46bea7e62c0deec64c90', 'ARTS15-20181013', 'Jason Yang', '# ARTS15-20181013
...
', '<h1 id="arts15-20181013">ARTS15-20181013</h1>
<blockquote>
...
<h2 id="review">Review</h2>
<h3 id="-apache-kafka-quickstart-https-kafka-apache-org-quickstart-"><a href="https://kafka.apache.org/quickstart">Apache Kafka Quickstart</a></h3>
...
<li><p>Topic</p>
<ul>
<li><p>属于特定类别的消息流称为主题。 数据存储在主题中。</p>
...
<h3 id="-kafka-streams-https-kafka-apache-org-documentation-streams-"><a href="https://kafka.apache.org/documentation/streams/">Kafka Streams</a></h3>
<p>使用kafka streams 进行消息的发布、处理、消费，比较神奇的流式操作使用起来很爽，不能打断点进行debug，但是可以通过peek()方法来查看运行时变量的值。</p>
<h2 id="tip">Tip</h2>
<ul>
<li><p>eureka ribbon 负载均衡遇到的坑</p>
<ul>
<li>服务下线时client会向eureka server 发送下线请求，服务端会把下线的客户端剔除，但是此时访问服务，依然会向已经下线的服务发送请求，抛出<code>connect failed</code>异常 ，原因是客户端有缓存，默认30s刷新一次，缓存刷新后，再次访问才会返回<code>no instance</code>异常信息。</li>
<li>解决方案暂时没有确定：<ul>
...
<li><p>复习 java 基础中的<code>值传递</code></p>
<ul>
<li><p>java 中的参数传递都是<code>值传递</code>，<code>值</code>包括基本类型和对象的引用，基本类型值和对象的引用存放在JVM内存的<code>栈内存</code>中，对象的实例本身是存放在<code>堆内存</code>中，栈内存中的引用指向堆内存中的对象实例。</p>
</li>
<li><p>传递基本类型值</p>
<pre><code class="lang-java">public static void test(int i){
    i = 1;
}
...
<li><p>传递对象的引用</p>
<pre><code class="lang-java">public static void test(User user){
    user.setName(&quot;jason&quot;);
}
...
<li><p>上面两个例子应该比较直观了，java 的参数传递始终都是值传递，int 类型i始终不会被改变；User 类型的 user 是引用值，其指向的堆内存里的User实例是可以修改的。</p>
</li>
</ul>
</li>
</ul>
...
', '第十五周 ARTS。这周约了几个好久不见的“狱友”聚餐（因为我们同在的上家公司是做司法行业业务系统的，所以调侃称为“狱友”），聊了很多，让我感触最深刻的就是关于读书和学习的思维力和金字塔原理，怎么样高效的读书学习也是有套路可寻的，掌握一些模式可以更快的掌握书中的有效信息，本周的Share分享几本相关的书，与大家共勉。', 'arts', NULL, NULL, 2, '2018-10-13 18:03:10.835+01', NULL);
...
INSERT INTO archive VALUES ('49460939a48f4f8cbc90b4e9d9d97eea', 'ARTS17-20181028', 'Jason Yang', '# ARTS17-20181028
...
为什么考虑分库分表，目前项目使用单库mysql存储数据，有部分表的数量达到了600多万，查询效率比较低，最近业务发展趋势比较好，后续这些表的数据可能会成倍增长，根据网上看的经验值，500万到1000万就要考虑是否分库分表。于是开始研究分库分表的东西，真的是不看不知道，一看吓一跳，要准备的东西太多了，分哪些表，按什么规则分，数据往那些表存，哪些需要存nosql数据库，分片数据查询，分布式事务。初步使用mycat数据库中间件来做个测试，目前刚了解了一点皮毛。想到前面那么多坑要踩，还是有点发憷哦。', '<h1 id="arts17-20181028">ARTS17-20181028</h1>
<blockquote>
...
<h2 id="review">Review</h2>
<h3 id="what-is-the-java-9-module-system-">What is the Java 9 Module System?</h3>
...
<p>为什么考虑分库分表，目前项目使用单库mysql存储数据，有部分表的数量达到了600多万，查询效率比较低，最近业务发展趋势比较好，后续这些表的数据可能会成倍增长，根据网上看的经验值，500万到1000万就要考虑是否分库分表。于是开始研究分库分表的东西，真的是不看不知道，一看吓一跳，要准备的东西太多了，分哪些表，按什么规则分，数据往那些表存，哪些需要存nosql数据库，分片数据查询，分布式事务。初步使用mycat数据库中间件来做个测试，目前刚了解了一点皮毛。想到前面那么多坑要踩，还是有点发憷哦。</p>
', '> Algorithm: Remove Element
>
> Review: What is the Java 9 Module System?
>
> Tip: Mycat 原理
>
> Share: 近期学习kafka和分库分表的一些感受', 'arts', NULL, NULL, 2, '2018-10-28 14:59:20.988+00', NULL);
...
INSERT INTO archive VALUES ('5e238b3a808848a0a08c5f9976672204', 'ARTS18-20181104', 'Jason Yang', '# ARTS18-20181104
...
#### [兼容MySQL的开源分布式数据库SequoiaDB在去哪儿网的实践](https://mp.weixin.qq.com/s?__biz=MzU0MDExOTUyMg==&mid=2247484366&idx=1&sn=a0266d3c8c639dac23d49fc910d28c19&scene=21#wechat_redirect)', '<h1 id="arts18-20181104">ARTS18-20181104</h1>
<ul>
...
<h2 id="tip">Tip</h2>
<h3 id="-sharding-sphere-http-shardingsphere-io-document-current-cn-"><a href="http://shardingsphere.io/document/current/cn/">Sharding-Sphere</a></h3>
...
<h4 id="-mysql-mycat-https-mp-weixin-qq-com-s-g-ewn9izer1abheg0f9phq-"><a href="https://mp.weixin.qq.com/s/g-ewn9iZeR1abhEG0f9phQ">三思！大规模MySQL运维陷阱之基于MyCat的伪分布式架构</a></h4>
<ul>
...
<h4 id="-mysql-sequoiadb-https-mp-weixin-qq-com-s-__biz-mzu0mdexotuymg-mid-2247484366-idx-1-sn-a0266d3c8c639dac23d49fc910d28c19-scene-21-wechat_redirect-"><a href="https://mp.weixin.qq.com/s?__biz=MzU0MDExOTUyMg==&amp;mid=2247484366&amp;idx=1&amp;sn=a0266d3c8c639dac23d49fc910d28c19&amp;scene=21#wechat_redirect">兼容MySQL的开源分布式数据库SequoiaDB在去哪儿网的实践</a></h4>
', '- Algorithm: Reverse String
- Review: shard
- Tip: sharding-sphere
- Share: mycat 从入门到放弃', 'arts', NULL, NULL, 2, '2018-11-11 10:21:10.118+00', NULL);
...
INSERT INTO archive VALUES ('8e2adc5b0bca419a9c5c50fd4066e1d0', 'ARTS10-20180908', 'Jason Yang', '## Algorithm
...
', '<h2 id="algorithm">Algorithm</h2>
<pre><code>package org.nocoder.leetcode.solution;
...
<blockquote>
<p>Apache Commons Chain<br><a href="https://www.baeldung.com/apache-commons-chain">https://www.baeldung.com/apache-commons-chain</a></p>
</blockquote>
...
<pre><code>public class HundredDenominationDispenser implements Command {
...
</code></pre><p><strong>Chain</strong></p>
<ul>
<li>Chain 是一组要顺序执行的 Command 的集合。</li>
</ul>
<pre><code>public class AtmWithdrawalChain extends ChainBase {
...
</code></pre><p>当Chain中的任何Command返回true时，它会强制Chain结束。</p>
...
<pre><code>public class AtmChainTest {
...
</code></pre><h2 id="tip">Tip</h2>
<h3 id="sessionstorage-localstorage">sessionStorage 和 localStorage</h3>
...
<h2 id="share">Share</h2>
<h3 id="chain-of-responsibility-pattern">Chain of Responsibility Pattern</h3>
<blockquote>
...
', '左耳听风，第十周ARTS', 'arts', NULL, NULL, 2, '2018-09-14 16:29:17.333+01', '2018-09-14 16:29:40.512+01');
INSERT INTO archive VALUES ('cce115545a544e76b59accf9cf764527', 'ARTS11-20180915', 'Jason Yang', '## Algorithm
...
', '<h2 id="algorithm">Algorithm</h2>
<h3 id="jewels-and-stones">Jewels and Stones</h3>
<pre><code class="lang-java">package org.nocoder.leetcode.solution;
...
<h2 id="review">Review</h2>
<h3 id="strategy-pattern-tutorial-with-java-examples">Strategy Pattern Tutorial with Java Examples</h3>
<blockquote>
...
<pre><code class="lang-java">//Strategy Interface
public interface CompressionStrategy {
  public void compressFiles(ArrayList&lt;File&gt; files);
}
</code></pre>
<pre><code class="lang-java">public class ZipCompressionStrategy implements CompressionStrategy {
  public void compressFiles(ArrayList&lt;File&gt; files) {
    //using ZIP approach
  }
}
</code></pre>
<pre><code class="lang-java">public class RarCompressionStrategy implements CompressionStrategy {
  public void compressFiles(ArrayList&lt;File&gt; files) {
    //using RAR approach
  }
}
</code></pre>
<pre><code class="lang-java">public class CompressionContext {
  private CompressionStrategy strategy;
  //this can be set at runtime by the application preferences
  public void setCompressionStrategy(CompressionStrategy strategy) {
    this.strategy = strategy;
...
<h2 id="tip">Tip</h2>
<h3 id="canal-server-docker">canal-server docker</h3>
...
<pre><code>-e canal.auto.scan=false \
-e canal.destinations=test \
-e canal.instance.master.address=127.0.0.1:3306  \
-e canal.instance.dbUsername=canal  \
-e canal.instance.dbPassword=canal  \
...
<p>策略模式提供了对“开闭原则”的完美支持，用户可以在不修改原有系统的基础上选择算法或行为，也可以灵活地增加新的算法或行为。<br>策略模式提供了管理相关的算法族的办法。<br>策略模式提供了可以替换继承关系的办法。<br>使用策略模式可以避免使用多重条件转移语句。</p>
...
<p>如果在一个系统里面有许多类，它们之间的区别仅在于它们的行为，那么使用策略模式可以动态地让一个对象在许多行为中选择一种行为。<br>一个系统需要动态地在几种算法中选择一种。<br>如果一个对象有很多的行为，如果不用恰当的模式，这些行为就只好使用多重的条件选择语句来实现。<br>不希望客户端知道复杂的、与算法相关的数据结构，在具体策略类中封装算法和相关的数据结构，提高算法的保密性与安全性。</p>
', '左耳听风，第十一周ARTS', 'arts', NULL, NULL, 2, '2018-09-15 18:28:05.911+01', NULL);
...
INSERT INTO archive VALUES ('bbae4c1c54634422829a65eb8db21cf5', 'ARTS13-20180929', 'Jason Yang', '## Algorithm
...
- 多线程
- 异常处理', '<h2 id="algorithm">Algorithm</h2>
<h2 id="review">Review</h2>
<h3 id="dynamic-proxies-in-java">Dynamic Proxies in Java</h3>
<blockquote>
<p><a href="https://www.baeldung.com/java-dynamic-proxies">https://www.baeldung.com/java-dynamic-proxies</a><br><a href="https://docs.oracle.com/javase/8/docs/technotes/guides/reflection/proxy.html">https://docs.oracle.com/javase/8/docs/technotes/guides/reflection/proxy.html</a></p>
</blockquote>
<h2 id="tip">Tip</h2>
<h2 id="share">Share</h2>
<blockquote>
<p>花了点时间总结一下来公司这些日子做的东西，自我感觉良好，跟着春哥学了不少东西，这半年还是很充实的。<br>记得有一次春哥笑着跟我说，“你要相信，别人能做出来的东西，你也可以。”</p>
</blockquote>
<h3 id="-">架构设计</h3>
<ul>
<li>领域驱动设计</li>
...
', '左耳听风，ARTS第十三周', 'arts', NULL, NULL, 2, '2018-09-29 15:53:53.071+01', '2018-09-29 16:01:29.083+01');
INSERT INTO archive VALUES ('b548dded89b14418821cea4bc49d5333', 'ARTS12-20180922', 'Jason Yang', '
...
<h2 id="review">Review</h2>
<h3 id="understanding-the-proxy-design-pattern">Understanding the Proxy Design Pattern</h3>
<blockquote>
...
<ul>
<li><p>Subject (Bank) : 银行通用接口</p>
</li>
<li><p>Real Subject (Bank Branch) : 实现接口的Real Subject</p>
</li>
<li><p>Proxy (ATM) : 代理类</p>
</li>
</ul>
<p>代理模式的适用场景：</p>
<ul>
...
<p>附上官方文档：<br><a href="https://docs.spring.io/spring-boot/docs/current/reference/html/build-tool-plugins-maven-plugin.html">68. Spring Boot Maven Plugin</a></p>
<h2 id="share">Share</h2>
<blockquote>
<p>代理模式（Proxy Pattern）：静态代理 - 最易懂的设计模式解析<br><a href="https://www.jianshu.com/p/a8aa6851e09e">https://www.jianshu.com/p/a8aa6851e09e</a></p>
</blockquote>
', ' 小伙伴们中秋节快乐！终于可以放松几天，这两天带着老婆孩子玩疯了，满成都到处跑，当然`ARTS`绝不能落下，只是晚到了一天。（算法题做的很烂，一点点调试出来的）', 'arts', NULL, NULL, 2, '2018-09-29 15:52:20.877+01', NULL);
INSERT INTO archive VALUES ('302b84398d8840f88b2f4bc9581de54a', '关于日志的那些事儿', 'Jason Yang', '这篇短文整理了关于日志的一些内容，耗叔要求的【目标是一篇“引经据典“的，“全面系统“的对日志的介绍】。结合一些参考链接和以往的经验，主要对软件开发过程中日志相关的知识进行梳理和学习。
...
<li>键值对<pre><code>      2017-07025 17:02:12 level=error message=&quot;connection refused&quot;
service=&quot;listener&quot; thread=125 customerid=776622 ip=34.124.233.12
queryid=45
</code></pre></li>
</ul>
...
logger.debug(&quot;Entry number: {} is {}&quot;, i, entry[i]);
</code></pre><p>上面两个语句输出的效果是一样的，但是性能却相差两个数量级，在关闭DEBUG日志时，它们的开销就不一样了，主要的影响在于字符串转换和字符串拼接上，无论是否生效，前者都会将变量转换为字符串并进行拼接，而后者则只会在需要时执行这些操作。</p>
<ul>
<li>避免无用的日志<ul>
...
<h2 id="-">参考文献</h2>
<ul>
<li><a href="https://zh.wikipedia.org/wiki/日志文件">日志文件wiki</a></li>
...
', '这篇短文整理了关于日志的一些内容，耗叔要求的【目标是一篇“引经据典“的，“全面系统“的对日志的介绍】。结合一些参考链接和以往的经验，主要对软件开发过程中日志相关的知识进行梳理和学习。', 'arts', NULL, NULL, 2, '2018-11-12 07:25:47.676+00', NULL);
...
INSERT INTO archive VALUES ('bee8e2ddb1e348cc988539c01b84506d', '近期接触的技术点总结', 'Jason Yang', '> 要花点时间总结一下来公司这些日子做的东西了，自我感觉良好，跟着春哥学了不少东西，这半年还是很充实的。
> 记得有一次春哥笑着跟我说，“你要相信，别人能做出来的东西，你也可以。”
...
', '<blockquote>
<p>要花点时间总结一下来公司这些日子做的东西了，自我感觉良好，跟着春哥学了不少东西，这半年还是很充实的。<br>记得有一次春哥笑着跟我说，“你要相信，别人能做出来的东西，你也可以。”</p>
</blockquote>
<h3 id="-">架构设计</h3>
<ul>
<li>领域驱动设计</li>
...
记得有一次春哥笑着跟我说，“你要相信，别人能做出来的东西，你也可以。”', 'arts', NULL, NULL, 2, '2018-09-29 15:06:05.327+01', '2018-09-29 16:00:03.306+01');
...
INSERT INTO comment VALUES ('deb199fb86374ae39064c9a87507beaa', 'bee8e2ddb1e348cc988539c01b84506d', 'dave', NULL, NULL, '厉害了我的哥', '2018-09-29 22:19:33.906', NULL);
INSERT INTO comment VALUES ('d687eb63114c46988f5d1474a7c0f486', 'bee8e2ddb1e348cc988539c01b84506d', 'jy', NULL, NULL, '厉害了我的哥', '2018-09-29 22:57:05.935', NULL);
INSERT INTO comment VALUES ('0a841ae8b81f4ffa97dc280b0b7525e4', 'bee8e2ddb1e348cc988539c01b84506d', 'Jason', NULL, NULL, '厉害了我的哥', '2018-09-29 23:03:12.933', NULL);
